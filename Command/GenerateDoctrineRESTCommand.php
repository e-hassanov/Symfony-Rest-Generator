...
namespace ehassanov\RestGeneratorBundle\Command;
...
use ehassanov\RestGeneratorBundle\Generator\DoctrineRESTGenerator;
use ehassanov\RestGeneratorBundle\Manipulator\RoutingManipulator;
...
EOT
            ...
            ->setName('ehassanov:generate:rest')
            ->setAliases(array('generate:ehassanov:rest'));
    }
    ...
    protected function interact(InputInterface $input, OutputInterface $output)
    {
        ...
        $output->writeln(
             $this->getHelper('formatter')->formatBlock('                                    REST API GENERATOR                                      ', 'bg=cyan;fg=white', true)
        ...
        $output->writeln(
            array(
                ...
                '   ║ ■ you defining it.                                                                   ║',
                '   ║                                                                                      ║',
                '   ║ You must use the shortcut notation like  <info>AcmeBlogBundle:Post</info>                         ║',
        ...
        $output->writeln(
            array(
                '',
                $this->getHelper('formatter')->formatBlock('Summary before generation', 'bg=green;fg=white', true),
                '',
    ...
    protected function updateRouting(QuestionHelper $questionHelper, InputInterface $input, OutputInterface $output, BundleInterface $bundle, $format, $entity, $prefix)
    {
        ...
        if (!$ret) {
            $help = sprintf(
                "        <comment>resource: \"@%s/Controller/%sController.php\"</comment>\n",
                $bundle->getName(),
                $entity
            );
